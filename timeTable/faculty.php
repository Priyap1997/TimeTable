...
<body>
  ...
  <div id="nav_bar">
    ...
    ?>
              <li class="limenu"><a href="manage.php?action=addcourse">Manage Courses</a></li>
      ...
      <div class="box">
          ...
          <div class="elements">
            <form method="post" action="courses.php?action=add">
               ...
               <select name="cName" id="allowed" class="stretch"  data-placeholder="courses" required>
                <?php
                foreach($db->query('SELECT * FROM ncourses') as $course)
                  echo "<option value=\"{$course['ncourse_name']} : {$course['ncourse_id']}  \">{$course['ncourse_name']} : {$course['ncourse_id']} </option>";
                ?>
              </select>
