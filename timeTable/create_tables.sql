...
-- --------------------------------------------------------
--
-- Table structure for table `ncourse`
--

DROP TABLE IF EXISTS `ncourses`;
CREATE TABLE IF NOT EXISTS `ncourses` (
 `ncourse_id` char(10) NOT NULL,
  `ncourse_name` varchar(100) NOT NULL,
	`ncourse_dept` varchar(100) NOT NULL
  ) ENGINE=InnoDB DEFAULT CHARSET=latin1;
-- --------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `faculty` (
  ...
  `level` enum('dean','hod','faculty','') NOT NULL  ,
  `dept_code` char(5) NOT NULL,
...
--
ALTER TABLE `allowed`
  ADD PRIMARY KEY (`course_name`,`batch_name`,`batch_dept`),
...
--
-- Indexes for table `ncourse`
--
ALTER TABLE `ncourses`
  ADD PRIMARY KEY (`ncourse_name`,`ncourse_dept`,`ncourse_id`),
  ADD KEY `ncourses_department` (`ncourse_dept`);
...
ALTER TABLE `slot_allocs`
  ADD PRIMARY KEY (`table_name`,`day`,`slot_num`,`room`,`course_id`,`fac_name`),
  ADD KEY `fk_course_id` (`course_id`),
  ADD KEY `fk_room` (`room`),
  ADD KEY `fk_slot` (`day`,`slot_num`),
  ADD KEY `fk_fac_name` (`fac_name`);
...
--
ALTER TABLE `allowed`
  ADD CONSTRAINT `batch_name` FOREIGN KEY (`batch_name`, `batch_dept`) REFERENCES `batches` (`batch_name`, `batch_dept`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `course_name` FOREIGN KEY (`course_name`) REFERENCES `courses` (`course_name`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
-- Constraints for table `ncourse`
--
  ALTER TABLE `ncourses`
  ADD CONSTRAINT `ncourses_department` FOREIGN KEY (`ncourse_dept`) REFERENCES `depts` (`dept_code`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `courses`
--
ALTER TABLE `courses`
  ADD CONSTRAINT `fac_id` FOREIGN KEY (`fac_id`) REFERENCES `faculty` (`uName`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `ncourses_id` FOREIGN KEY (`course_id`) REFERENCES `ncourses` (`ncourse_id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `slot_allocs`
  ...
  ADD CONSTRAINT `fk_fac` FOREIGN KEY (`fac_name`) REFERENCES `faculty` (`fac_name`) ON DELETE CASCADE ON UPDATE CASCADE,
