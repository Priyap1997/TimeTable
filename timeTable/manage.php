...
<body style="white-space:nowrap">
  <div id="header">
    ...
    <div id="header_text">TimeTable</div>
  </div>
  ...
  <div id="nav_bar">
    <ul class="main_menu" id="main_menu">
      ...
      <li class="limenu"><a href="manage.php?action=courses">Manage Courses</a></li>
  ...
  <?php elseif(valueCheck('action','batches')) : ?>
    ...
    <div class="box">
     ...
     <?php elseif(valueCheck('action','courses')) : ?>
    <div class="box">
      <div class="boxbg"></div>
      <div class="information"><div class="icon add"></div></div>
      <div class="title">Add Courses</div>
      <div class="elements">
        <form method="post" action="addcourse.php?action=add">
                   <input type="text" name="ncourse_id" class="styled uInfo" required pattern="[^:]{2,30}" title="2 to 30 alphanumeric characters" placeholder="Course_id" />

          <input type="text" name="ncourse_name" class="styled uInfo" required pattern="[^:]{2,100}" title="2 to 100 alphanumeric characters" placeholder="Course_Name" />
          
       <select name="ncourse_dept" class="stretch" data-placeholder="Choose Department..." required>
            <option label="Choose Department..."></option>
            <?php
            foreach($db->query('SELECT * FROM depts') as $dept)
              echo "<option value=\"{$dept['dept_code']}\">{$dept['dept_name']} ({$dept['dept_code']})</option>";
    ...
    <div class="box">
      ...
      <div class="elements">
        <form method="post" action="addcourse.php?action=delete" class="confirm">
          <select name="ncourse" class="updateSelect stretch"  data-placeholder="Choose Course..." required>
            <option label="Choose Course..."></option>
            <?php
            foreach($db->query('SELECT * FROM ncourse') as $ncourse)
              echo "<option value=\"{$ncourse['ncourse_name']} : {$ncourse['ncourse_id']}\">{$ncourse['ncourse_name']} : {$ncourse['ncourse_id']}</option>";
            ?>
          </select>
          <input type="hidden" id="confirm_msg" value="Are you sure you want to delete the selected batch?">
          <div class="blocktext info"></div>
