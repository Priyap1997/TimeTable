...
foreach ($courses as $course) {
  ...
  $que= $db->query("SELECT course_id,count(*) as batches FROM 
       allowed batches 
       group by course_id") ;
  //$query->execute([$course['course_id']]);
  $conflicts = $que->fetchall();
  foreach ($conflicts as $conflict) 
  {
    $query = $db->prepare('SELECT day,slot_num FROM slot_allocs where (table_name=? AND course_id=?) or fac_name=?');
    $query->execute([$current['table_name'],$conflict['course_id'],$_SESSION['faculty']]);
...
if(valueCheck('action','saveSlots'))
{
  ...
  try
  {
    foreach ($_POST as $slotStr => $course_room) 
    {
      ...
      $query->execute([$current['table_name'],$slot[0],$slot[1],$room,$course,$_SESSION['faculty']]);
    }
  }
